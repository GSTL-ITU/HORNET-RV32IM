...
reg port0_ack;
always @(posedge port0_wb_clk_i or negedge port0_wb_rst_ni)
begin
    if(!port0_wb_rst_ni)
        port0_ack <= 1'b0;
...
reg port1_ack;
always @(posedge port1_wb_clk_i or negedge port1_wb_rst_ni)
begin
    if(!port1_wb_rst_ni)
        port1_ack <= 1'b0;
